...
router.post('/', (req, res) => {
  const { title, description = '', status = 'da_fare', checklist = [], tags = [] } = req.body;
  ...
  const info = db
    .prepare('INSERT INTO projects (title, description, status, checklist, tags) VALUES (?, ?, ?, ?, ?)')
    .run(title, description, finalStatus, JSON.stringify(checklist), JSON.stringify(tags));
...
router.put('/:id', (req, res) => {
  ...
  const { title, description, status, checklist, tags } = req.body;
  const finalStatus = status && VALID_STATUSES.includes(status) ? status : existing.status;
  db.prepare(
    "UPDATE projects SET title = ?, description = ?, status = ?, checklist = ?, tags = ?, updated_at = datetime('now') WHERE id = ?"
  ).run(
